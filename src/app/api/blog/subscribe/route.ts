...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingSubscriber = await SubscriberService.checkExistingSubscriber(email, 'blog');
    
    if (existingSubscriber) {
      if (existingSubscriber.verified) {
        return NextResponse.json(
          { 
            error: 'You are already subscribed to the blog newsletter!',
    ...
    const subscriber = await SubscriberService.createSubscriber(email, 'blog', verificationToken);
